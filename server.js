...
const { Student, sequelize } = require('./src/server/models/student');
...
app.use('/static', express.static(path.join(__dirname, 'src/server/src/main/resources/static')));
...
app.get('/api/login', checkAlreadyAuthenticated, (req, res) => {
  res.sendFile(path.join(__dirname, 'src/server/src/main/resources/templates/ekran-logowania.html'));
});
...
app.get('/api/reset-password', (req, res) => {
  res.sendFile(path.join(__dirname, 'src/server/src/main/resources/templates/reset-hasla.html'));
});

app.get('/api/reset-password/info', (req, res) => {
  res.sendFile(path.join(__dirname, 'src/server/src/main/resources/templates/info-o-resecie.html'));
});

app.get('/api/new-password', (req, res) => {
  const token = req.query.token;
  res.sendFile(path.join(__dirname, 'src/server/src/main/resources/templates/nowe-haslo.html'));
});

app.get('/api/new-password/info', (req, res) => {
  res.sendFile(path.join(__dirname, 'src/server/src/main/resources/templates/reset-udany.html'));
});
...
app.post('/api/reset-password', async (req, res) => {
  ...
  const { data, error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: 'https://localhost:3000/api/new-password',
  });
